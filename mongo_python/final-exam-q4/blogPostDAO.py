...
class BlogPostDAO:
    ...
    def increment_likes(self, permalink, comment_ordinal):
        post = self.posts.find_one({'permalink': permalink})
        #email = post['comments'][comment_ordinal]['email']
        if 'num_likes' not in post['comments'][comment_ordinal]:
                   likes = 1
        else:
                   likes = post['comments'][comment_ordinal]['num_likes']+1
        comments_id = 'comments.'+str(comment_ordinal)+'.num_likes'
	try:
            last_error = self.posts.update({'permalink': permalink}, {'$set': {comments_id: likes}}, upsert=False,
                                           manipulate=False, safe=True)
